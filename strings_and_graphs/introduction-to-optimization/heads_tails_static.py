...
def create_coin(face):
    ...
    return VGroup(coin, get_coin_label(face, coin), create_drop_shadow(coin))
...
def create_less_than_sign(cluster1, cluster2):
    ...
    return MathTex(r"\boldsymbol{\prec_{C}}").move_to(point).scale(COIN_RADIUS * 2)
...
def create_equals_sign(cluster1, cluster2):
    ...
    point = Point(location=[x_coord, y_coord, 0])
    # TODO decide on bolding and size
    return MathTex(r"\boldsymbol{\sim_{C}}").move_to(point).scale(COIN_RADIUS * 2)
...
def create_section(face_clusters):
    ...
    return VGroup(c1, c2, c3, create_equals_sign(c1, c2), create_equals_sign(c2, c3))
...
class HeadsTails(Scene):
    def construct(self):
        ...
        lo_arrow = Arrow(
            stroke_width=dg.ARROW_STROKE_WIDTH,
            start=RIGHT,
            end=LEFT,
        ...
        s2 = create_section([["H", "T", "T"], ["T", "H", "T"], ["T", "T", "H"]])
        s2.move_to([left_side + width / 3 * 1, 0, 0])

        s3 = create_section([["H", "H", "T"], ["T", "H", "H"], ["H", "T", "H"]])
