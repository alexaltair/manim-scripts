...
class AltairLogo(Scene):
    def construct(self):
        ...
        big_square = Square(
            side_length=4 * basic_unit, color=TEXT_COLOR, stroke_width=STROKE_WIDTH
        )
        big_circle = Circle(
            radius=2 * basic_unit, color=TEXT_COLOR, stroke_width=STROKE_WIDTH
        )
        ...
        top_circle = Circle(
            radius=0.44 * basic_unit, color=TEXT_COLOR, stroke_width=STROKE_WIDTH
        )
        top_circle.shift(0.28 * LEFT * basic_unit)
        top_circle.shift(big_square.get_top() + [0, top_circle.radius, 0])

        med_left_circle = Circle(
            radius=0.56 * basic_unit, color=TEXT_COLOR, stroke_width=STROKE_WIDTH
        )
        med_left_circle.shift(2.12 * LEFT * basic_unit)
        med_left_circle.shift(big_square.get_bottom() + [0, med_left_circle.radius, 0])

        med_right_circle = Circle(
            radius=0.52 * basic_unit, color=TEXT_COLOR, stroke_width=STROKE_WIDTH
        )
        med_right_circle.shift(2.32 * RIGHT * basic_unit)
        med_right_circle.shift(
            big_square.get_bottom() + [0, med_right_circle.radius, 0]
        )

        small_left_circle = Circle(
            radius=med_left_circle.radius / 2,
            color=TEXT_COLOR,
            stroke_width=STROKE_WIDTH,
        )
        small_left_circle.shift(1.12 * LEFT * basic_unit)
        small_left_circle.shift(
            big_square.get_bottom() + [0, small_left_circle.radius, 0]
        )

        small_right_circle = Circle(
            radius=small_left_circle.radius, color=TEXT_COLOR, stroke_width=STROKE_WIDTH
        )
        small_right_circle.shift([-small_left_circle.get_x(), 0, 0])
        small_right_circle.shift(
            big_square.get_bottom() + [0, small_right_circle.radius, 0]
        )
        ...
        for path in arc2.get_subpaths():
            shape.add_subpath(path)
        med_right_circle_almost_left = med_right_circle.get_left() * [
            1,
            1.08,
            1,
        ]  # CALIBRATE
        ...
        shape.add_line_to(
            ORIGIN + [0.6 * small_right_circle.get_center()[0], crossbar_bottom, 0]
        )  # CALIBRATE
        shape.add_line_to(
            ORIGIN + [0.7 * small_left_circle.get_center()[0], crossbar_bottom, 0]
        )  # CALIBRATE
        small_left_circle_almost_left = small_left_circle.get_left() * [
            0.97,
            0.9,
            1,
        ]  # CALIBRATE
