...
class BitsOfOptimization(Scene):
    def construct(self):
        ...
        x_axis_text = Tex(
            r"$\leftarrow$ States ordered according to $C \rightarrow$"
        ).next_to(ax, DOWN)
        ...
        lognormal_curve = ax.plot(
            lambda x: lognormal(x), x_range=[0.01, 4], color=DARK_BLUE
        )

        def current_area():
            samples, dx = np.linspace(
                start=current_state.get_value(), stop=20, num=50, retstep=True
            )
