...
class BayesNetNew(Scene):
    def construct(self):
        x_node = create_node(
            fill_color=N_NODE_COLOR, label="X", label_shift=(-0.05, 0, 0)
        )
        y_node = create_node(fill_color=S_NODE_COLOR, label="Y")
        a_node = create_node(
            fill_color=R_NODE_COLOR, label="A", label_shift=(-0.05, 0.1, 0)
        )
        ...
        left_group_caption = Tex("(a) A blind policy", color=OUTER_COLOR).next_to(
            left_group, DOWN, buff=0.8
        )
        right_group_caption = Tex("(b) A sighted policy", color=OUTER_COLOR).next_to(
            right_group, DOWN, buff=0.8
        )
...
class BarChartNew(Scene):
    def construct(self):
        ...
        labels = chart.get_bar_labels(
            color=OUTER_COLOR, label_constructor=MathTex, font_size=LABELS_FONT_SIZE
        )

        x_label = Tex("Number of Bits Known", font_size=CAPTION_FONT_SIZE).next_to(
            chart, DOWN, buff=CAPTION_BUFF
        )
        y_label = (
            Tex("Output Entropy", font_size=CAPTION_FONT_SIZE)
            .rotate(PI / 2)
            .next_to(chart, LEFT, buff=CAPTION_BUFF)
        )
